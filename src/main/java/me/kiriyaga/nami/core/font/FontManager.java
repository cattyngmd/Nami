...
public class FontManager {
    ...
    public void drawText(GuiGraphics context, Component text, int x, int y, boolean shadow, int color) {
        if (shadow) {
            ...
            context.drawString(rendererProvider.getRenderer(), text, x, y, ColorUtils.darken(ColorUtils.fromRGBA(color), 50).getRGB(), false);
            matrices.popMatrix();
        }
    ...
    public void drawText(GuiGraphics context, Component text, int x, int y, boolean shadow) {
        if (shadow) {
            ...
            matrices.pushMatrix();
            matrices.translate(0.5f, 0.5f);
            context.drawString(rendererProvider.getRenderer(), ColorUtils.darken(text, 50), x, y, 0xFFFFFFFF, false);
            matrices.popMatrix();
        }
    ...
    public void drawText(GuiGraphics context, String text, int x, int y, boolean shadow) {
        if (shadow) {
            ...
            matrices.pushMatrix();
            matrices.translate(0.5f, 0.5f);
            context.drawString(rendererProvider.getRenderer(), text, x, y, ColorUtils.darken(ColorUtils.fromRGBA(0xFFFFFFFF), 50).getRGB(), false);
            matrices.popMatrix();
        }
