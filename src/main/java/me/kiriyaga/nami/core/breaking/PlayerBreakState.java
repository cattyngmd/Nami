...
import me.kiriyaga.nami.util.entity.PlayerUtils;
...
public class PlayerBreakState {
    ...
    private void renderProgress(Render3DEvent event, BreakTask task) {
        ...
        RenderUtil.drawBoxLines(box, new Color(200, 150, 0, 255), MODULE_MANAGER.getStorage().getByClass(BreakHighlightModule.class).fill.get(), true, 1.5f);
    }

    public void startBreak(BlockPos pos, Direction face, float speed) {
        if (current.isActive() && !current.pos.equals(pos)) {
            if (MODULE_MANAGER.getStorage().getByClass(SpeedMineModule.class).doubleMine.get()) {
