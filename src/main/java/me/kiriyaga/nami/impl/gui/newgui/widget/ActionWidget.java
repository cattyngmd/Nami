...
import me.kiriyaga.nami.impl.gui.newgui.base.PanelRenderer;
...
public class ActionWidget {
    ...
    private final int lineHeight = FONT_SERVICE.getHeight() + 1;
    ...
    private void recalcSize() {
        ...
        for (ActionItem item : items) {
            int w = FONT_SERVICE.getWidth(item.getLabel());
    ...
    public void render(GuiGraphics context, Font textRenderer, int mouseX, int mouseY) {
        ...
        int drawY = y + padding;
        for (ActionItem item : items) {
            int textWidth = FONT_SERVICE.getWidth(item.getLabel());

            int color = (mouseX >= x + padding && mouseX <= x + padding + textWidth &&
                    mouseY >= drawY && mouseY <= drawY + lineHeight) ? FEATURE_SERVICE.getStorage().getByClass(ColorFeature.class).getStyledTextColor(255).getRGB() : FEATURE_SERVICE.getStorage().getByClass(ColorFeature.class).getStyledTextSecondColor(255).getRGB();

            FONT_SERVICE.drawText(context, item.getLabel(), x + padding, drawY, color, true);
            drawY += lineHeight;
        }
    }
