...
public class PingManagerModule extends Module {
    ...
    public PingManagerModule() {
        ...
        if (!this.isEnabled())
            this.toggle();
        smoothingStrength.setShowCondition(() -> fastLatency.get());
        unstableConnectionTimeout.setShowCondition(() -> fastLatency.get());
        keepAliveInterval.setShowCondition(() -> fastLatency.get());
    }
