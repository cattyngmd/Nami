...
public class AutoMineModule extends Module {
    ...
    private final BoolSetting doubleMime = addSetting(new BoolSetting("DoubleMine", true));
    private final EnumSetting<Mode> mode = addSetting(new EnumSetting<>("Logic", Mode.GRIM));
    private final BoolSetting face = addSetting(new BoolSetting("face", true));

    public AutoMineModule() {
        super("AutoMine", "Manages which blocks shoulf SpeedMine module mine.", ModuleCategory.of("Combat"));
    ...
    public void onTick(PreTickEvent event) {
        ...
        SpeedMineModule m = MODULE_MANAGER.getStorage().getByClass(SpeedMineModule.class);

/*        if (m.currentTask != null)
            CHAT_MANAGER.sendPersistent("1", "instantremine: " + m.currentTask.isInstantRemine());*/
    ...
    private void sendToSpeedMine(BlockPos main) {
        SpeedMineModule speedMine = MODULE_MANAGER.getStorage().getByClass(SpeedMineModule.class);
