...
public class AuraModule extends Module {
    public final DoubleSetting attackRange = addSetting(new DoubleSetting("Attack Range", 2.6, 1, 5));
    public final DoubleSetting rotateRange = addSetting(new DoubleSetting("Rotate Range", 4, 2, 8)); // новое поле
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        Entity target = getTarget(rotateRange.get());

        if (target == null || MINECRAFT.player.squaredDistanceTo(target) > attackRange.get() * attackRange.get()) {
            return;
        }
    ...
    private Entity getTarget(double range) {
        ...
        return hostiles.stream()
                .filter(e -> e.isAlive() && e.squaredDistanceTo(player) <= range * range)
