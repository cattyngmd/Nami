...
public class ConfigEntry extends BaseEntry {
    ...
    public ConfigEntry(String name, ConfigMeta meta) {
        ...
        if (meta == null) {
            this.displayText = CAT_FORMAT.format("Name: {global}"+name + " {gray}Saved: {global}unknown" + "{gray}Author: {global}unknown" + "{gray}Version: {global}unknown");
        } else {
            this.displayText = CAT_FORMAT.format("Name: {global}"+name + " {gray}Saved: {global}" + meta.mode().name() +" {gray}Author: {global}" + meta.author() + " {gray}Version: {global}" + meta.clientVersion());
        }
    }
